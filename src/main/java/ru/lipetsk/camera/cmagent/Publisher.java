...
import ru.lipetsk.camera.cmagent.core.service.RTMPService;
...
import ru.lipetsk.camera.cmagent.net.rtp.RTP;
import ru.lipetsk.camera.cmagent.net.rtp.RTPPacket;
import ru.lipetsk.camera.cmagent.net.rtp.RTPWrapper;
 ...
 */
public class Publisher extends Thread implements IEventListener {
    ...
    public Publisher(String fmsHost, int fmsPort, String fmsApp, String ipAddress, String uri) {
    ...
    public void run() {
        ...
        this.rtspClient.connect(this.ipAddress, 554, this.uri, new Credentials("admin", "admin"));
    ...
    public void onEventHandle(IEvent event) {
        switch (event.getName()) {
            case Events.RTMP_CREATE_STREAM:
                this.rtmpClient.publishStream("TestStream");
            ...
            case Events.RTMP_NO_DATA:
                System.out.println("RTMP NO DATA");

                System.out.println(this.rtspClient.isConnected());
    ...
    private void onH264VideoFrameHandle(NALUnit nalUnit) {
        System.out.println("NAL Unit: " + nalUnit.getFuIndicator().getType());
